    ...
    getRoleRelatedKRA :function(component, event, helper){
         ...
         action.setCallback(this, function(response) {
            ...
            if(state === "SUCCESS" && responseData.isSuccess){
                var JSONResponse = JSON.parse(responseData.response);
                console.log(JSONResponse);
                if(JSON.parse(JSONResponse[0]['TARGETS']).length > 0){
               		component.set("v.lstTargets", JSON.parse(JSONResponse[0]['TARGETS']));
                    component.set("v.showTargetTable", true);
                }
                 if(JSONResponse[1]['ALLOW_SUBMITION']){
                    component.set("v.allowSubmition", JSONResponse[1]['ALLOW_SUBMITION']);
                }
                if(JSONResponse[2]['ALLOW_MANAGER']){
                    component.set("v.AllowManagerEdit", JSONResponse[2]['ALLOW_MANAGER']);
                }
             }
             else if(state === "SUCCESS" && !responseData.isSuccess){
                  this.showErrorToast(component, event,'No Tagets are defined for selected duration');
             }
        });
        $A.enqueueAction(action);  
    },
